...
for module in ["train"]:
    ...
    # odn0 = np.loadtxt("./result/{}_0_{}_{}_list.txt".format(dataset,opt))
        x = np.loadtxt("./result15/{}_xent_{}_epoch_list.txt".format(dataset,opt))
        odn1 = np.loadtxt("./result15/{}_1_{}_train_list.txt".format(dataset,opt,module))
        odn0 = np.loadtxt("./result15/{}_0_{}_test_list.txt".format(dataset,opt,module))
        ...
        plt.plot(x,odn1-odn0,label="s-mdNet_{}".format(opt),color=color[idx+1],linewidth=1.5)
        idx += 2
        idx %= 6
        plt.xlabel('Epoch')
        plt.ylabel('Accuracy (%)')
